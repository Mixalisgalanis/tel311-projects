...
%  The following command loads the dataset. The dataset is then visualized.
load('ex1_1_data1.mat');
figure(1); subplot(1,2,1); plot(X(:, 1),X(:, 2), '.'); grid on;
fprintf('Program paused. Press enter to continue.\n'); pause;

%  Before running PCA, it is important to first stardardize X using 
%  feautureNormalize. I then proceed to visualize the normalized samples
[X_norm, mu, sigma] = featureNormalize(X);
figure(1); subplot(1,2,2); plot(X_norm(:, 1),X_norm(:, 2), '.'); grid on;
fprintf('Program paused. Press enter to continue.\n'); pause;
...
%  Compute mu, the mean of each feature in form of a vector
mu = mean(X_norm);
...
S_diag = diag(S);
[eigval, idx] = sort(S_diag, 1, 'descend');
eigvec = U(:, idx);
PCvariance = eigval/sum(eigval);
...
figure(2); plot(X_norm(:, 1), X_norm(:, 2), '.'); grid on;
